...
void  Filt_Pw( Word16 *DataBuff, Word16 *Dpnt, Word16 Start, PWDEF Pw )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        DataBuff[Start+(Word16)i] = round_2( Acc0 ) ;
    }
...
*Line, Word16 Sfc )
{
    ...
    for ( k = 0 ; k < (int) Hb ; k ++ ) {
        ...
        for ( i = 0 ; i < SubFrLen ; i ++ ) {
            ...
            FltBuf[ClPitchOrd-1][i] = round_2( Acc0 ) ;
        }
...
Word16   Comp_Info( Word16 *Buff, Word16 Olp, Word16 *Gain, Word16 *ShGain)
{
    ...
    *ShGain = Vec_Norm( Buff, (Word16) (PitchMax+Frame) ) ;
    ...
    for ( i = (int)Olp-3 ; i <= (int)Olp+3 ; i ++ ) {
        ...
        //if ( Acc0 > Acc1 ) {  //YC Debug, Change for match asembly
        if ( Acc0 >= Acc1 ) {
            Acc1 = Acc0 ;
            Indx = (Word16) i ;
        }
...
PFDEF Get_Ind( Word16 Ind, Word16 Ten, Word16 Ccr, Word16 Enr )
{
    ...
    if ( Acc1 > Acc0 ) {
        ...
        if ( Acc1 >= Acc0 )
			...
			// YT modified to match assembly implementation of 16bit division
            //Exp = div_l( Acc1, Exp ) ; 
			Exp = div_l( Acc1&0xFFFF0000, Exp ) ;
...
void  Filt_Lpf( Word16 *Tv, Word16 *Buff, PFDEF Pf, Word16 Sfc )
{
    ...
    for ( i = 0 ; i < SubFrLen ; i ++ ) {
        ...
        Acc0 = L_mac( Acc0, Buff[PitchMax+(int)Sfc*SubFrLen+(int)Pf.Indx+i],
                                                                Pf.Gain ) ;
        Tv[(int)Sfc*SubFrLen+i] = round_2( Acc0 ) ;
    }
                 ...
                 Word16 y[], Word16 *code_shift, Word16 *sign)
{
  ...
  */
	...
	//YT Modified
    for (i0 = 0; i0 < SubFrLen-1; i0 += STEP) {
        ps0  = Dn[i0];
        ps0a = Dn[i0+1];
        alp0 = *ptr_ri0i0++;
