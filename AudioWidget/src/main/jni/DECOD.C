...
Flag    Decod( Word16 *DataBuff, char *Vinp, Word16 Crc )
{
    ...
    Word32   Senr ;
    Word16   QntLpc[SubFrames*LpcOrder] ;//40samples
    Word16   AcbkCont[SubFrLen] ;//60

    Word16   LspVect[LpcOrder] ;//10
    Word16   Temp[PitchMax+Frame] ;//145+240 = 385
    Word16  *Dpnt ;

    LINEDEF  Line ;
    PFDEF    Pf[SubFrames] ;//4
        ...
        */
        ...
        if ( DecStat.Ecount == (Word16) 0 ) {
            ...
            for ( i = 0 ; i < SubFrames ; i ++ ) {
                ...
                /* Generate the fixed codebook excitation for a         subframe. (Text: Section 3.5) */
