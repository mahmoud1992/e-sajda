...
//    }
  ...
  */
    for ( j = 0 ; j < LpcOrder ; j ++ ) {
		...
		//Lsp[j] = add( Lsp[j], Tmp ) ;
	    Acc0 = (((Word32)Lsp[j])<<16);
		 Acc0 = L_mac(Acc0,PrevLsp[j],Lprd);
        Lsp[j] = ((Word16)(Acc0>>16)) ;
    }
...
void  LsptoA( Word16 *Lsp )
{
  ...
  */
        ...
        Acc0 = L_mac( Acc0, Tmp, add( shl( (Word16)(Lsp[i] & 0x007f),(Word16)8 ), (Word16) 0x0080 ) ) ;
        Acc0 = L_shl( Acc0, (Word16) 1 ) ;
        Lsp[i] = negate( round_2( Acc0 ) ) ;
    }
  ...
  */
    for ( i = 0 ; i < LpcOrder/2 ; i ++ ) {
        ...
        Lsp[LpcOrder-1-i] = negate( round_2( Acc1 ) ) ;
    }
